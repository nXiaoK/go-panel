...
func (l *dtlsListener) Init(md md.Metadata) (err error) {
	...
	// pion/dtls v3 由调用方显式控制握手上下文；先完成有界握手，再进入
	// PROXY protocol、统计和限流包装，防止半开连接长期占用监听资源。
	ln = &handshakeTimeoutListener{Listener: ln, timeout: 30 * time.Second}
