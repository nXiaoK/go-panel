...
load_sub_panel_config() {
    [[ -f "$SUB_PANEL_FILE" ]] || return 1
    # shellcheck disable=SC1090
    source "$SUB_PANEL_FILE" 2>/dev/null || return 1
    return 0
}
...
sub_panel_protocol_name() {
    case "$1" in
        vless|vless-xhttp|vless-ws|vless-vision) echo "vless" ;;
        vmess|vmess-ws) echo "vmess" ;;
        trojan) echo "trojan" ;;
        ss2022|ss-legacy) echo "ss" ;;
        snell|snell-v5) echo "snell" ;;
        socks) echo "socks5" ;;
        *) echo "$1" ;;
    esac
}
...
sub_panel_network() {
    case "$1" in
        vless-ws|vmess-ws) echo "ws" ;;
        vless-xhttp) echo "xhttp" ;;
        *) echo "tcp" ;;
    esac
}
...
sub_panel_security() {
    case "$1" in
        vless|vless-xhttp) echo "reality" ;;
        vless-ws|vless-vision|vmess-ws|trojan) echo "tls" ;;
        *) echo "" ;;
    esac
}

sub_panel_sanitize_identifier() {
    local value="$1"
    value=$(echo "$value" | tr -c 'A-Za-z0-9_.-' '-' | sed -E 's/^-+//; s/-+$//; s/-+/-/g')
    printf '%s\n' "${value:0:80}"
}

sub_panel_legacy_source_host() {
    local host
    host=$(hostname 2>/dev/null || echo "unknown")
    sub_panel_sanitize_identifier "$host"
}

sub_panel_read_machine_id() {
    local id path
    for path in /etc/machine-id /var/lib/dbus/machine-id /sys/class/dmi/id/product_uuid; do
        [[ -r "$path" ]] || continue
        id=$(head -n 1 "$path" 2>/dev/null | tr -d '[:space:]')
        [[ -z "$id" || "$id" =~ ^0+$ ]] && continue
        printf '%s\n' "$id"
        return 0
    done
...
sub_panel_source_host() {
    local server="${1:-}" id=""
    if [[ -n "${SUB_PANEL_NODE_ID:-}" ]]; then
        id="$SUB_PANEL_NODE_ID"
    elif [[ -r "$SUB_PANEL_NODE_ID_FILE" ]]; then
        id=$(head -n 1 "$SUB_PANEL_NODE_ID_FILE" 2>/dev/null | tr -d '[:space:]')
    else
        id=$(openssl rand -hex 16 2>/dev/null || sub_panel_read_machine_id 2>/dev/null || date +%s%N)
        mkdir -p "$CFG" 2>/dev/null || true
        umask 077
        printf '%s\n' "$id" > "$SUB_PANEL_NODE_ID_FILE" 2>/dev/null || true
    fi
    if [[ -z "$id" ]]; then
        id="$(sub_panel_legacy_source_host)-${server:-unknown}"
    fi
    printf 'node-%s\n' "$(sub_panel_sanitize_identifier "$id")"
}

sub_panel_protocol_label() {
    case "$1" in
        ss2022|ss-legacy|ss2022-shadowtls) echo "SS" ;;
        snell|snell-v5|snell-shadowtls|snell-v5-shadowtls) echo "Snell" ;;
        vless|vless-xhttp|vless-ws|vless-vision) echo "VLESS" ;;
        vmess|vmess-ws) echo "VMess" ;;
        trojan) echo "Trojan" ;;
        socks) echo "SOCKS5" ;;
        ...
        anytls) echo "AnyTLS" ;;
        naive) echo "Naive" ;;
        *) echo "$(sub_panel_protocol_name "$1" | tr '[:lower:]' '[:upper:]')" ;;
    esac
}

sub_panel_ip_info_field() {
    local ip="$1" field="$2" json=""
    if [[ -n "$ip" ]]; then
        json=$(curl -sf --connect-timeout 3 "http://ip-api.com/json/${ip}?fields=status,countryCode,isp,org,asname" 2>/dev/null || true)
    else
        json=$(curl -sf --connect-timeout 3 "http://ip-api.com/json/?fields=status,countryCode,isp,org,asname" 2>/dev/null || true)
    fi
    [[ -z "$json" ]] && return 0
    echo "$json" | jq -r --arg field "$field" 'if .status == "success" then .[$field] // empty else empty end' 2>/dev/null
}

sub_panel_provider_label() {
    local raw="$1" lower cleaned
    raw=$(echo "$raw" | tr -d '\r\n')
    lower=$(echo "$raw" | tr '[:upper:]' '[:lower:]')
    case "$lower" in
        *greencloud*) echo "GreenCloud" ;;
        *alibaba*|*aliyun*|*"ali cloud"*) echo "Aliyun" ;;
        *tencent*) echo "TencentCloud" ;;
        *amazon*|*aws*) echo "AWS" ;;
        *google*) echo "GoogleCloud" ;;
        *oracle*) echo "OracleCloud" ;;
        *microsoft*|*azure*) echo "Azure" ;;
        *digitalocean*) echo "DigitalOcean" ;;
        *vultr*|*"constant company"*) echo "Vultr" ;;
        *linode*|*akamai*) echo "Linode" ;;
        *hetzner*) echo "Hetzner" ;;
        *ovh*) echo "OVH" ;;
        *cloudflare*) echo "Cloudflare" ;;
        *hivelocity*) echo "Hivelocity" ;;
        *contabo*) echo "Contabo" ;;
        *hosthatch*) echo "HostHatch" ;;
        *bandwagon*|*it7*) echo "BandwagonHost" ;;
        *racknerd*) echo "RackNerd" ;;
        *gcore*) echo "Gcore" ;;
        *leaseweb*) echo "Leaseweb" ;;
        *dedipath*) echo "DediPath" ;;
        *quadranet*) echo "QuadraNet" ;;
        *netcup*) echo "Netcup" ;;
        *scaleway*) echo "Scaleway" ;;
        *ionos*) echo "IONOS" ;;
        *upcloud*) echo "UpCloud" ;;
        *lightnode*) echo "LightNode" ;;
        *kamatera*) echo "Kamatera" ;;
        *)
            cleaned=$(echo "$raw" \
                | sed -E 's/^AS[0-9]+[[:space:]]*//I; s/[,(].*$//; s/\b(LLC|LTD|LIMITED|INC|CORP|CORPORATION|CO|COMPANY|NETWORK|NETWORKS|HOSTING|VPS)\b//Ig; s/[^A-Za-z0-9]+//g')
            echo "${cleaned:-Cloud}"
            ;;
    esac
}

sub_panel_provider_name() {
    local ip="$1" raw=""
    raw=$(sub_panel_ip_info_field "$ip" "isp")
    [[ -z "$raw" ]] && raw=$(sub_panel_ip_info_field "$ip" "org")
    [[ -z "$raw" ]] && raw=$(sub_panel_ip_info_field "$ip" "asname")
    if [[ -z "$raw" ]]; then
        if [[ -n "$ip" ]]; then
            raw=$(curl -sf --connect-timeout 3 "https://ipinfo.io/${ip}/json" 2>/dev/null | jq -r '.org // empty' 2>/dev/null)
        else
            raw=$(curl -sf --connect-timeout 3 "https://ipinfo.io/json" 2>/dev/null | jq -r '.org // empty' 2>/dev/null)
        fi
    fi
    sub_panel_provider_label "$raw"
}

sub_panel_region_code() {
    local ip="$1" region=""
    region=$(sub_panel_ip_info_field "$ip" "countryCode")
    [[ -z "$region" ]] && region=$(get_ip_country "$ip" 2>/dev/null || true)
    region=$(echo "$region" | tr -d '[:space:]' | tr '[:lower:]' '[:upper:]')
    [[ -z "$region" ]] && region="XX"
    echo "$region"
}

sub_panel_node_name() {
    local protocol="$1" server="$2" provider="${3:-}" region="${4:-}" protocol_label="${5:-}"
    [[ -z "$provider" ]] && provider=$(sub_panel_provider_name "$server")
    [[ -z "$region" ]] && region=$(sub_panel_region_code "$server")
    [[ -z "$protocol_label" ]] && protocol_label=$(sub_panel_protocol_label "$protocol")
    printf '%s-%s-%s\n' "$provider" "$region" "$protocol_label"
}

sub_panel_protocol_config() {
    local protocol="$1"
    if db_exists "xray" "$protocol"; then
        db_get "xray" "$protocol"
    elif db_exists "singbox" "$protocol"; then
        db_get "singbox" "$protocol"
    fi
}

sub_panel_build_payload() {
    local protocol="$1" cfg="$2"
    local server port actual_port master_port name normalized network security link provider region protocol_label source_host legacy_source_host external_id legacy_external_id
    server=$(sub_panel_server_ip)
    [[ -z "$server" ]] && return 1
    ...
    normalized=$(sub_panel_protocol_name "$protocol")
    network=$(sub_panel_network "$protocol")
    security=$(sub_panel_security "$protocol")
    provider=$(sub_panel_provider_name "$server")
    region=$(sub_panel_region_code "$server")
    protocol_label=$(sub_panel_protocol_label "$protocol")
    name=$(sub_panel_node_name "$protocol" "$server" "$provider" "$region" "$protocol_label")
    source_host=$(sub_panel_source_host "$server")
    legacy_source_host=$(sub_panel_legacy_source_host)
    external_id="${source_host}-${protocol}"
    legacy_external_id="${legacy_source_host}-${protocol}"

    local uuid sni short_id public_key path password username method psk snell_version flow allow_insecure udp_enabled
    uuid=$(echo "$cfg" | jq -r '.uuid // empty')
    ...
    psk=$(echo "$cfg" | jq -r '.psk // empty')
    snell_version=$(echo "$cfg" | jq -r '.version // empty')
    [[ -z "$snell_version" && "$protocol" == "snell" ]] && snell_version=4
    [[ -z "$snell_version" && "$protocol" == "snell-v5" ]] && snell_version=5
    flow=""
    allow_insecure=false
    udp_enabled=true

    # SS + ShadowTLS 没有独立的 UDP 入口，NaïveProxy/HTTP CONNECT 也不支持 UDP。
    # Snell 的普通 UDP 会封装在 TCP 会话中，即使叠加 ShadowTLS 仍可工作。
    case "$protocol" in
        ss2022-shadowtls|naive) udp_enabled=false ;;
    esac

    case "$protocol" in
        vless)
            flow="xtls-rprx-vision"
            link=$(gen_vless_link "$server" "$actual_port" "$uuid" "$public_key" "$short_id" "$sni")
            ;;
        vless-xhttp)
            link=$(gen_vless_xhttp_link "$server" "$actual_port" "$uuid" "$public_key" "$short_id" "$sni" "$path")
            ;;
        vless-ws)
            allow_insecure=true
            link=$(gen_vless_ws_link "$server" "$actual_port" "$uuid" "$sni" "$path")
            ;;
        vless-vision)
            flow="xtls-rprx-vision"
            allow_insecure=true
            link=$(gen_vless_vision_link "$server" "$actual_port" "$uuid" "$sni")
            ;;
        vmess)
            link=$(gen_vmess_link "$server" "$actual_port" "$uuid")
            ;;
        vmess-ws)
            allow_insecure=true
            link=$(gen_vmess_ws_link "$server" "$actual_port" "$uuid" "$sni" "$path")
            ;;
        trojan)
            allow_insecure=true
            link=$(gen_trojan_link "$server" "$actual_port" "$password" "$sni")
            ;;
        ss2022)
            link=$(gen_ss2022_link "$server" "$actual_port" "$method" "$password")
            ;;
        ss-legacy)
            link=$(gen_ss_legacy_link "$server" "$actual_port" "$method" "$password")
            ;;
        snell)
            password="$psk"
            link=$(gen_snell_link "$server" "$actual_port" "$psk" "4")
            ;;
        snell-v5)
            password="$psk"
            link=$(gen_snell_v5_link "$server" "$actual_port" "$psk" "5")
            ;;
        socks)
            link=$(gen_socks_link "$server" "$actual_port" "$username" "$password")
            ;;
    esac

    local options
    options=$(jq -cn \
        --arg source "vless-server.sh" \
        --arg originalProtocol "$protocol" \
        --arg originalPort "$port" \
        --arg provider "$provider" \
        --arg region "$region" \
        --arg protocolLabel "$protocol_label" \
        --arg sourceHost "$source_host" \
        --arg legacySourceHost "$legacy_source_host" \
        '{source:$source,originalProtocol:$originalProtocol,originalPort:$originalPort,provider:$provider,region:$region,protocolLabel:$protocolLabel,sourceHost:$sourceHost,legacySourceHost:$legacySourceHost}')

    jq -n \
        --arg externalId "$external_id" \
        --arg sourceHost "$source_host" \
        --arg legacyExternalId "$legacy_external_id" \
        --arg legacySourceHost "$legacy_source_host" \
        --arg name "$name" \
        --arg protocol "$normalized" \
        --arg server "$server" \
        ...
        --argjson snellVersion "${snell_version:-0}" \
        --argjson allowInsecure "$allow_insecure" \
        --argjson udp "$udp_enabled" \
        --arg link "$link" \
        --arg options "$options" \
        '{externalId:$externalId,sourceHost:$sourceHost,legacyExternalId:$legacyExternalId,legacySourceHost:$legacySourceHost,name:$name,protocol:$protocol,server:$server,port:$port,uuid:$uuid,username:$username,password:$password,method:$method,sni:$sni,network:$network,security:$security,path:$path,flow:$flow,publicKey:$publicKey,shortId:$shortId,fingerprint:$fingerprint,snellVersion:$snellVersion,allowInsecure:$allowInsecure,udp:$udp,link:$link,options:$options,status:1}'
}

sub_panel_report_protocol() {
    local protocol="$1"
    load_sub_panel_config || return 0
    local base api_key cfg payload endpoint
    base=$(sub_panel_base_url) || return 0
    api_key="${SUB_PANEL_API_KEY:-}"
    [[ -z "$api_key" ]] && return 0
    cfg=$(sub_panel_protocol_config "$protocol")
    [[ -z "$cfg" ]] && return 0
    payload=$(sub_panel_build_payload "$protocol" "$cfg") || return 0
    endpoint="$base/api/v1/sub/node/report"
    if curl -fsS --max-time 10 -H "Content-Type: application/json" -H "X-API-Key: $api_key" -d "$payload" "$endpoint" >/tmp/vless-sub-panel-report.log 2>&1; then
        _ok "已上报订阅节点到 Flux Panel: $protocol"
    else
        _warn "订阅节点上报失败: $protocol ($(cat /tmp/vless-sub-panel-report.log 2>/dev/null | tail -1))"
    fi
    rm -f /tmp/vless-sub-panel-report.log 2>/dev/null
}

sub_panel_delete_report() {
    local payload="$1" label="${2:-节点协议}"
    load_sub_panel_config || return 0
    local base api_key endpoint
    base=$(sub_panel_base_url) || return 0
    api_key="${SUB_PANEL_API_KEY:-}"
    [[ -z "$api_key" ]] && return 0
    endpoint="$base/api/v1/sub/node/delete-report"
    if curl -fsS --max-time 10 -H "Content-Type: application/json" -H "X-API-Key: $api_key" -d "$payload" "$endpoint" >/tmp/vless-sub-panel-delete.log 2>&1; then
        _ok "已同步删除 Flux Panel 订阅节点: $label"
    else
        _warn "同步删除 Flux Panel 订阅节点失败: $label ($(cat /tmp/vless-sub-panel-delete.log 2>/dev/null | tail -1))"
    fi
    rm -f /tmp/vless-sub-panel-delete.log 2>/dev/null
}

sub_panel_delete_protocol() {
    local protocol="$1"
    [[ -z "$protocol" ]] && return 0
    local server payload source_host legacy_source_host
    server=$(sub_panel_server_ip)
    source_host=$(sub_panel_source_host "$server")
    legacy_source_host=$(sub_panel_legacy_source_host)
    payload=$(jq -n \
        --arg externalId "${source_host}-${protocol}" \
        --arg sourceHost "$source_host" \
        --arg legacyExternalId "${legacy_source_host}-${protocol}" \
        --arg legacySourceHost "$legacy_source_host" \
        --arg protocol "$(sub_panel_protocol_name "$protocol")" \
        --arg server "$server" \
        --arg cleanupMode "protocol" \
        '{externalId:$externalId,sourceHost:$sourceHost,legacyExternalId:$legacyExternalId,legacySourceHost:$legacySourceHost,protocol:$protocol,server:$server,cleanupMode:$cleanupMode}')
    sub_panel_delete_report "$payload" "$protocol"
}

sub_panel_delete_all() {
    local server payload source_host legacy_source_host
    server=$(sub_panel_server_ip)
    source_host=$(sub_panel_source_host "$server")
    legacy_source_host=$(sub_panel_legacy_source_host)
    payload=$(jq -n \
        ...
        --arg legacySourceHost "$legacy_source_host" \
        --arg server "$server" \
        --arg cleanupMode "server" \
        '{sourceHost:$sourceHost,legacySourceHost:$legacySourceHost,server:$server,cleanupMode:$cleanupMode}')
    sub_panel_delete_report "$payload" "$source_host"
}

sub_panel_report_all() {
    local installed
    installed=$(get_installed_protocols)
    [[ -z "$installed" ]] && { _warn "未安装任何协议"; return 0; }
    local protocol
    for protocol in $installed; do
        sub_panel_report_protocol "$protocol"
    done
}

configure_sub_panel_report() {
    echo ""
    _line
    echo -e "  ${W}Flux Panel 订阅上报配置${NC}"
    _line
    load_sub_panel_config || true
    echo -e "  ${D}当前面板: ${SUB_PANEL_URL:-未配置}${NC}"
    echo ""
    read -rp "  面板地址 (例如 http://1.2.3.4:6365): " panel_url
    read -rp "  订阅 API Key: " api_key
    [[ -z "$panel_url" || -z "$api_key" ]] && { _err "面板地址/API Key 不能为空"; return 1; }
    save_sub_panel_config "$panel_url" "$api_key"
    _ok "订阅上报配置已保存: $SUB_PANEL_FILE"
    read -rp "  是否立即上报所有已安装协议? [Y/n]: " yn
    [[ "$yn" =~ ^[nN]$ ]] || sub_panel_report_all
}
...
flux_panel_bind_cli() {
    ...
    init_log
    init_db
    [[ -z "$panel_url" || -z "$api_key" ]] && {
        _err "用法: $0 --flux-panel-bind <panel_url> <api_key> [server_ip]"
        exit 1
    }
    if [[ -n "$server_ip" ]]; then
        SUB_PANEL_SERVER="$server_ip"
    fi
    save_sub_panel_config "$panel_url" "$api_key"
    if [[ -n "$server_ip" ]]; then
        {
            echo "SUB_PANEL_SERVER=$(sub_panel_quote "$server_ip")"
        } >> "$SUB_PANEL_FILE"
    fi
    _ok "Flux Panel 上报配置已保存: $SUB_PANEL_FILE"
    sub_panel_report_all
}
...
register_protocol() {
    ...
    if [[ -n "$config_json" ]]; then
        ...
        sub_panel_report_protocol "$protocol" 2>/dev/null || true
    fi
}

unregister_protocol() {
    local protocol=$1
    sub_panel_delete_protocol "$protocol" 2>/dev/null || true
...
generate_xray_config() {
    ...
    # 配置生成后会拉起 xray 进程；缺二进制时先补装，避免“协议已写入库但服务启动失败”
    if ! check_cmd xray || ! _xray_supports_x25519; then
...
gen_vless_link() {
    ...
    # VLESS 的 UDP 通过当前流式连接转发；packetEncoding=none 用于让客户端保留 UDP 能力。
    printf '%s\n' "vless://${uuid}@${ip}:${port}?encryption=none&security=reality&type=tcp&packetEncoding=none&sni=${sni}&fp=chrome&pbk=${pbk}&sid=${sid}&flow=xtls-rprx-vision#${name}"
}
...
gen_vless_xhttp_link() {
    ...
    # XHTTP 仍在既有传输中承载 UDP，不要求服务端额外开放同端口 UDP。
    printf '%s\n' "vless://${uuid}@${ip}:${port}?encryption=none&security=reality&type=xhttp&packetEncoding=none&sni=${sni}&fp=chrome&pbk=${pbk}&sid=${sid}&path=$(urlencode "$path")&mode=auto#${name}"
}
...
gen_qr() {
    ...
    if command -v qrencode >/dev/null 2>&1; then
        if printf '%s' "$value" | qrencode -t ANSIUTF8; then
            return 0
        fi
        # Binary exists but execution failed — do not claim it is uninstalled.
        printf '%s\n' "$value"
        printf '%s\n' "本地二维码生成失败，请检查 qrencode 安装与终端能力后重试"
        return 1
    fi
    printf '%s\n' "$value"
    printf '%s\n' "未安装 qrencode；请使用系统包管理器在本机安装后重试"
}
...
gen_vless_ws_link() {
    ...
    printf '%s\n' "vless://${uuid}@${ip}:${port}?encryption=none&security=tls&sni=${sni}&type=ws&packetEncoding=none&host=${sni}&path=$(urlencode "$path")&allowInsecure=1#${name}"
}
...
gen_vless_vision_link() {
    ...
    printf '%s\n' "vless://${uuid}@${ip}:${port}?encryption=none&security=tls&sni=${sni}&type=tcp&packetEncoding=none&flow=xtls-rprx-vision&allowInsecure=1#${name}"
}
...
# 显示协议真实的 UDP 承载方式和需要放行的端口。
# 这里只给出命令，不自动修改主机防火墙；云厂商安全组仍需在控制台单独放行。
show_udp_setup_hint() {
    local protocol="$1" port="$2"

    echo ""
    _line
    echo -e "  ${W}UDP 支持说明${NC}"

    case "$protocol" in
        vless|vless-xhttp|vless-ws|vless-vision|vmess|vmess-ws|trojan|anytls)
            echo -e "  ${G}✓ UDP 转发已由协议在现有连接中承载。${NC}"
            echo -e "  ${D}服务端只显示 TCP 监听属于正常现象，不需要额外开放同端口 UDP。${NC}"
            ;;
        snell)
            echo -e "  ${G}✓ Snell v3+ 会自动提供 UDP Relay。${NC}"
            echo -e "  ${D}普通 UDP 封装在现有 TCP 会话中；只看到 TCP 监听属于正常现象。${NC}"
            echo -e "  ${D}snell.conf 不存在 udp=true 配置项，也无需额外开放同端口 UDP。${NC}"
            ;;
        snell-v5)
            echo -e "  ${G}✓ 普通 UDP 会自动通过现有 TCP 会话转发。${NC}"
            echo -e "  ${D}Snell v5 仅在 Surge 的 QUIC Proxy 加速模式下额外使用同端口 UDP。${NC}"
            if [[ -n "$port" ]]; then
                if ss -uln 2>/dev/null | grep -q ":${port} "; then
                    echo -e "  ${G}✓ 已检测到可供 QUIC Proxy 使用的 UDP 端口 $port。${NC}"
                else
                    echo -e "  ${Y}⚠ 未检测到 UDP 端口 $port；普通 UDP-over-TCP 不受影响，但 QUIC Proxy 不可用。${NC}"
                fi
                echo -e "  ${D}如需 QUIC Proxy，请在主机防火墙和云安全组开放 UDP 端口 $port：${NC}"
                echo -e "  ${C}ufw allow $port/udp${NC}"
                echo -e "  ${C}iptables -A INPUT -p udp --dport $port -j ACCEPT${NC}"
            fi
            echo -e "  ${D}snell.conf 不存在 udp=true 配置项，UDP Relay 无需额外配置。${NC}"
            ;;
        ss2022|ss-legacy|socks)
            echo -e "  ${G}✓ 服务端配置已启用 UDP Relay。${NC}"
            if [[ -n "$port" ]]; then
                if ss -uln 2>/dev/null | grep -q ":${port} "; then
                    echo -e "  ${G}✓ 已检测到 UDP 端口 $port 正在监听。${NC}"
                else
                    echo -e "  ${Y}⚠ 未检测到 UDP 端口 $port；请先检查服务状态与生成配置。${NC}"
                fi
                echo -e "  ${Y}请在主机防火墙和云安全组同时开放 TCP+UDP 端口 $port。${NC}"
                echo -e "  ${D}ufw 示例:${NC}"
                echo -e "  ${C}ufw allow $port/tcp${NC}"
                echo -e "  ${C}ufw allow $port/udp${NC}"
                echo -e "  ${D}iptables UDP 示例:${NC}"
                echo -e "  ${C}iptables -A INPUT -p udp --dport $port -j ACCEPT${NC}"
            fi
            ;;
        hy2|tuic)
            echo -e "  ${G}✓ 该协议直接使用 UDP 传输。${NC}"
            if [[ -n "$port" ]]; then
                echo -e "  ${Y}请在主机防火墙和云安全组开放 UDP 端口 $port。${NC}"
                echo -e "  ${C}ufw allow $port/udp${NC}"
                echo -e "  ${C}iptables -A INPUT -p udp --dport $port -j ACCEPT${NC}"
            fi
            ;;
        snell-shadowtls|snell-v5-shadowtls)
            echo -e "  ${G}✓ Snell 的 UDP 会封装在现有 TCP 会话中，可继续通过 ShadowTLS 转发。${NC}"
            echo -e "  ${D}只看到 ShadowTLS 的 TCP 监听属于正常现象；Snell v5 的原生 QUIC Proxy 加速在此组合中不可用。${NC}"
            ;;
        ss2022-shadowtls)
            echo -e "  ${Y}⚠ 当前 SS2022 + ShadowTLS 组合只转发 TCP，不能提供 Shadowsocks 原生 UDP Relay。${NC}"
            echo -e "  ${D}如需 UDP，请安装不带 ShadowTLS 的 Shadowsocks，或改用 Hysteria2/TUIC。${NC}"
            ;;
        naive)
            echo -e "  ${Y}⚠ NaïveProxy/HTTP CONNECT 不支持 UDP Relay。${NC}"
            ;;
    esac

    # 协议本身不支持 UDP 时，不显示会让用户误以为能补救的客户端开关提示。
    case "$protocol" in
        ss2022-shadowtls|naive) ;;
        *) echo -e "  ${D}Shadowrocket 还需在「设置 → UDP → 启用转发」以及订阅/节点详情中开启 UDP 转发。${NC}" ;;
    esac
    _line
}
...
# 协议是否由 Xray 核心承载（与 XRAY_PROTOCOLS / PROTO_KIND 保持一致，避免漏装）
is_xray_protocol() {
    local protocol="$1"
    [[ " $XRAY_PROTOCOLS " == *" $protocol "* ]] && return 0
    [[ "${PROTO_KIND[$protocol]:-}" == "xray" ]] && return 0
    # ss2022 后端仍由 xray 进程提供
    [[ "$protocol" == "ss2022-shadowtls" ]] && return 0
    return 1
}

# 创建/导入协议前确保对应运行时二进制已安装。
# 历史上 SOCKS 等 Xray 协议未走 install_xray 分支，会导致配置写完但服务创建/启动失败。
ensure_protocol_runtime() {
    local protocol="$1"
    [[ -z "$protocol" ]] && { _err "ensure_protocol_runtime: 协议名为空"; return 1; }

    if is_xray_protocol "$protocol"; then
        install_xray || return 1
    fi
    ...
    case "$protocol" in
        ...
        snell-v5-shadowtls)
            ...
            ;;
        ss2022-shadowtls)
            # xray 已在 is_xray_protocol 分支处理
            install_shadowtls || return 1
            ;;
        anytls)
            install_anytls || return 1
            ;;
        naive)
            install_naive || return 1
            ;;
    esac
    return 0
}
...
show_single_protocol_info() {
    ...
    case "$protocol" in
        ...
        ss2022)
            ...
            echo ""
            echo -e "  ${Y}Surge 配置:${NC}"
            echo -e "  ${C}${country_code}-SS2022 = ss, ${config_ip}, ${display_port}, encrypt-method=${method}, password=${password}, udp-relay=true${NC}"
            echo ""
            echo -e "  ${Y}Loon 配置:${NC}"
        ...
        ss-legacy)
            ...
            echo ""
            echo -e "  ${Y}Surge 配置:${NC}"
            echo -e "  ${C}${country_code}-SS = ss, ${config_ip}, ${display_port}, encrypt-method=${method}, password=${password}, udp-relay=true${NC}"
            echo ""
            echo -e "  ${Y}Loon 配置:${NC}"
        ...
        ss2022-shadowtls)
            ...
            echo ""
            echo -e "  ${Y}Loon 配置:${NC}"
            echo -e "  ${C}${country_code}-SS2022-ShadowTLS = Shadowsocks, ${config_ip}, ${display_port}, ${method}, \"${password}\", shadow-tls-password=${stls_password}, shadow-tls-sni=${sni}, shadow-tls-version=3${NC}"
            echo -e "  ${Y}注意: 当前 ShadowTLS 组合模式仅代理 TCP，不能把 udp=true/udp-relay=true 写入客户端。${NC}"
            ;;
        snell|snell-v5)
            echo -e "  PSK: ${G}$psk${NC}"
    ...
    if [[ -n "$ip_addr" ]]; then
        ...
        if [[ "$protocol" != "snell-shadowtls" && "$protocol" != "snell-v5-shadowtls" && "$protocol" != "ss2022-shadowtls" ]]; then
            if [[ "$protocol" == "socks" ]]; then
                ...
                echo ""
                if command -v qrencode >/dev/null 2>&1; then
                    echo -e "  ${C}二维码 (本地生成):${NC}"
                    echo -e "  ${G}$(gen_qr "$socks_link")${NC}"
                else
                    echo -e "  ${D}未安装 qrencode；请使用系统包管理器在本机安装后显示二维码${NC}"
                fi
            else
                echo -e "  ${C}分享链接:${NC}"
                echo -e "  ${G}$link${NC}"
    ...
    if [[ "$has_cert_protocol" == "true" ]]; then
        ...
        if [[ "$web_service_running" == "true" && -f "$CFG/sub.info" ]]; then
            ...
            echo -e "  ${Y}Clash / Shadowrocket / Clash Verge:${NC}"
    ...
    show_udp_setup_hint "$protocol" "$display_port"
...
do_uninstall() {
    ...
    sub_panel_delete_all 2>/dev/null || true
...
do_install_server() {
    ...
    # 按协议元数据安装运行时（含 SOCKS 等原先遗漏的 Xray 协议）
    _info "检测并安装协议运行时..."
    ensure_protocol_runtime "$protocol" || return
...
external_link_to_surge() {
    ...
    case "$type" in
        ...
        ss)
            local method=$(echo "$json" | jq -r '.method')
            local password=$(echo "$json" | jq -r '.password')
            # Surge 的 Shadowsocks UDP Relay 默认关闭，必须显式声明。
            echo "$name = ss, $server, $port, encrypt-method=$method, password=$password, udp-relay=true"
            ;;
        socks)
            local username=$(echo "$json" | jq -r '.username // empty')
            local password=$(echo "$json" | jq -r '.password // empty')
...
gen_clash_sub() {
    ...
    udp: true"
                ;;
            snell|snell-v5)
                # Snell v3 及以上会在服务端自动提供 UDP；Mihomo/Shadowrocket 需要此节点标记。
                local clash_snell_version="${version:-4}"
                [[ "$protocol" == "snell-v5" ]] && clash_snell_version="${version:-5}"
                [[ -n "$server_ip" ]] && proxy="  - name: \"$name\"
    type: snell
    server: \"$server_ip\"
    port: $port
    psk: \"$psk\"
    version: $clash_snell_version
    reuse: true
    udp: true"
                ;;
            socks)
    ...
    for protocol in $installed; do
        ...
        case "$protocol" in
            ...
            trojan)
                ...
                ;;
            ss2022)
                [[ -n "$server_ip" ]] && proxy="$name = ss, $server_ip, $port, encrypt-method=$method, password=$password, udp-relay=true"
                ;;
            ss-legacy)
                [[ -n "$server_ip" ]] && proxy="$name = ss, $server_ip, $port, encrypt-method=$method, password=$password, udp-relay=true"
                ;;
        ...
        if [[ -f "$CFG/sub.info" ]]; then
            case $choice in
                ...
                5)
                    ...
                    _pause
                    ;;
                6) configure_sub_panel_report; _pause ;;
                7) sub_panel_report_all; _pause ;;
                0) return ;;
            esac
        else
            case $choice in
                ...
                3) configure_sub_panel_report; _pause ;;
                4) sub_panel_report_all; _pause ;;
                0) return ;;
            esac
        fi
    done
