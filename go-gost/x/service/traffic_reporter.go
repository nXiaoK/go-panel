...
// HTTPS、端口及子路径必须与 WebSocket 连接保持一致，禁止回退成固定明文 HTTP。
func SetHTTPReportURL(addr string, secret string) error {
	values := url.Values{}
	values.Set("secret", secret)
	var err error
	httpReportURL, err = panelurl.BuildHTTPURL(addr, "/flow/upload", values)
	if err != nil {
		return err
	}
	configReportURL, err = panelurl.BuildHTTPURL(addr, "/flow/config", values)
	if err != nil {
		return err
	}
