...
ensure_dependencies() {
  ...
  if [ -f /etc/os-release ]; then
    ...
    case "$ID" in
      ubuntu|debian)
        $SUDO_CMD apt update && $SUDO_CMD apt install -y nftables curl iperf3 conntrack
        ;;
      centos|rhel|rocky|almalinux|fedora)
        if command -v dnf >/dev/null 2>&1; then
          $SUDO_CMD dnf install -y nftables curl iperf3 conntrack-tools
        else
          $SUDO_CMD yum install -y nftables curl iperf3 conntrack-tools
        fi
        ;;
      alpine)
        $SUDO_CMD apk add --no-cache nftables curl iperf3 conntrack-tools
        ;;
      arch|manjaro)
        $SUDO_CMD pacman -Sy --noconfirm nftables curl iperf3 conntrack-tools
        ;;
      *)
        echo "❌ 当前发行版暂未自动适配，请先手动安装 nftables/curl/iperf3/conntrack"
        exit 1
        ;;
    esac
  else
    echo "❌ 无法识别系统，请先手动安装 nftables/curl/iperf3/conntrack"
    exit 1
  fi
}
