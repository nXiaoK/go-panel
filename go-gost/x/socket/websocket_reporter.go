...
func (w *WebSocketReporter) connect() error {
	...
	// 使用最新的本地配置重建 URL，并向面板上报节点实际使用的历史入口。
	configuredAddr := w.addr
	if strings.TrimSpace(cfg.Addr) != "" {
		configuredAddr = cfg.Addr
	}
	currentURL, panelBaseURL, err := buildReporterWebSocketURL(
		configuredAddr, w.secret, w.version, cfg.Http, cfg.Tls, cfg.Socks,
	)
	if err != nil {
		return fmt.Errorf("构建 WebSocket URL 失败: %v", err)
	}
	w.addr = panelBaseURL
...
func (w *WebSocketReporter) routeCommand(cmd CommandMessage) {
	...
	case "ReconcileManagedRuntime":
		var req reconcileManagedRuntimeRequest
		req, err = decodeReconcileManagedRuntimeRequest(cmd.Data)
		if err == nil {
			err = reconcileManagedRuntime(req)
		}
		response.Type = "ReconcileManagedRuntimeResponse"
	...
	case "UpgradeNode":
		err = w.handleUpgradeNode(cmd.Data)
		response.Type = "UpgradeNodeResponse"
...
func buildReporterWebSocketURL(addr, secret, version string, httpPort, tlsPort, socksPort int) (string, string, error) {
	panelBaseURL, err := panelurl.NormalizeBase(addr)
	if err != nil {
		return "", "", err
	}
	values := url.Values{}
	...
	values.Set("http", strconv.Itoa(httpPort))
	values.Set("tls", strconv.Itoa(tlsPort))
	values.Set("socks", strconv.Itoa(socksPort))
	// panelUrl 仅包含面板基址，不包含节点 secret；面板会在成功握手后保存并供后续升级复用。
	values.Set("panelUrl", panelBaseURL)
	wsURL, err := panelurl.BuildWebSocketURL(panelBaseURL, values)
	if err != nil {
		return "", "", err
	}
	return wsURL, panelBaseURL, nil
}

// StartWebSocketReporterWithConfig 使用配置字段启动 WebSocket 报告器。
func StartWebSocketReporterWithConfig(addr string, secret string, http int, tls int, socks int, version string) (*WebSocketReporter, error) {
	fullURL, panelBaseURL, err := buildReporterWebSocketURL(addr, secret, version, http, tls, socks)
	if err != nil {
		return nil, err
	}

	// 日志只输出不含 secret 的基址，避免节点密钥进入 systemd 日志。
	fmt.Printf("🔗 WebSocket 面板地址: %s\n", panelBaseURL)

	reporter := NewWebSocketReporter(fullURL, secret)
	// 保存规范化基址、secret 和版本供重连及节点本地升级使用。
	reporter.addr = panelBaseURL
